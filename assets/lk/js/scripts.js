...
function hpost(url,array_id,array_text){
  var post = '';''
  var all_text = '';
  if(array_id.length != 0){
    array_id.forEach(function(item, i, array_id) {

      $('[id='+item+']').each(function (index, el){
        var vall = $(el).val();
        if($(el).attr('type') == 'checkbox'){ //checkbox
          if($(el).prop('checked'))
            vall = 'true';
          else
            vall = 'false';
        }
        if($(el).hasClass('form-control-wrap')) { //select
          vall = $(el).children('select').val();
        }
        all_text += item;
        if( $('[id='+item+']').length > 1)
          all_text += '['+index+']';
        all_text += '=' + encodeURIComponent( vall )
        if(  $('[id='+item+']').length > 1 &&  $('[id='+item+']').length != index+1 ) {
          all_text += '&';
        }
      });

      if( array_id.length > 1 && array_id.length != i+1 ) {
        all_text += '&';
      }
    });
    post = all_text;
  ...
  if(array_text.length != 0){
    array_text.forEach(function(item, i, array_text) {
    if( post != '' ) post += '&';
    post += item[0] + '=' + encodeURIComponent( item[1] );
        if( array_text.length > 1 && array_text.length != i+1 ) {
            post += '&'
        }
    });
  }
...
function btn_spinner (btn){
    $(btn).html('<div><span class="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span></div>');
    $(btn).prop('disabled', true);
}


function interval_months (y,m,d,subtract){
    ...
    while (i <= subtract) { 
      date.setDate(date.getDate() - 1);  
      //alert( date.getDate() +' '+ months[date.getMonth()]);
      result_arr.unshift(date.getDate() +' '+ months[date.getMonth()]);
      i++;
    }
    return result_arr;
};

function interval_hour (hour){
    var result_arr = [];
    let i = 0;
    while (i <= hour) { 
        if(i < 10)
            result_arr.push( '0' + i + ':00');
        else
            result_arr.push(i + ':00');    
      i++;
    }
    return result_arr;   
}

function pagination(current,all,max,id){
    if(all <= max)
        return;

    var page_count = Math.ceil(all/max+1);
    var content = '<ul class="pagination justify-content-center justify-content-md-start">';
    var ds;
    for (let i = 1; i < page_count; i++) {
        if(i == 1){
           if(current == 1)
               ...
               ds = '';
           content += '<li class="page-item '+ds+'"><a class="page-link" href="#" aria-label="Пред"><span aria-hidden="true">«</span></a></li>';
        ...
        if(i > current - 3) {
            if(i < current + 3){
                ...
                content +=  '<li class="page-item '+ds+'"><a class="page-link" href="#" onclick="alert(1);return false;">'+i+'</a></li>';
            }
        }
        if(i == page_count - 1){
            ...
            content += '<li class="page-item '+ds+'"><a class="page-link" href="#" aria-label="След"><span aria-hidden="true">»</span></a></li>';
        }
    }
    content += '</ul></div>';
    $('#'+id).html(content);
...
function favorite($star){
    if(!$($star).find('em').toggleClass('ni-star').toggleClass('ni-star-fill')){
        $($star).find('em').toggleClass('ni-star-fill').toggleClass('ni-star');
    }
}

function domains($domain){
    if($($domain).find('em').hasClass('ni-toggle-on')){
        $($domain).find('em').toggleClass('ni-toggle-on').toggleClass('ni-toggle-off');
    }
    else{
        if($($domain).find('em').hasClass('ni-toggle-off')){
            $('.ni-toggle-on').toggleClass('ni-toggle-on').toggleClass('ni-toggle-off');
            $($domain).find('em').toggleClass('ni-toggle-off').toggleClass('ni-toggle-on');
        }
    }
}
